...
```tsx
<Card
  className={formData.role === role ? "border-primary bg-primary/5 border-2" : ""}
  ...
  <CardContent>
    <div className="flex items-start gap-3">
      ...
      <div className="h-5 w-5 rounded-full border-2">
        {formData.role === role && <div className="h-2 w-2 bg-white" />}
      </div>
...
<div className="flex flex-wrap gap-2">
  <Button onClick={() => setRoleFilter("")}>Tümü ({stats.total})</Button>
...
<Card className="transition-shadow hover:shadow-md">
  <CardHeader>
    <div className="flex items-start gap-4">
      {/* Avatar */}
      <div className="from-primary/20 h-14 w-14 rounded-full bg-gradient-to-br">
