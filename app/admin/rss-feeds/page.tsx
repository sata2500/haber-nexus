...
export default function RssFeedsPage() {
  ...
  const handleDelete = async (id: string) => {
    ...
    if (feed._count.articles > 0) {
      ...
      confirmMessage +=
        "Makaleleri silmek isterseniz makale yönetim sayfasından manuel olarak silebilirsiniz."
    }
  ...
  const handleScan = async (feedId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json()
        alert(
          data.message || "Tarama başlatıldı. Sonuçları tarama geçmişinden kontrol edebilirsiniz."
        )
      ...
      {filteredFeeds.length === 0 ? (
        <Card>
          <CardContent className="py-12 text-center">
            ...
            {!searchQuery && (
              <Button onClick={() => router.push("/admin/rss-feeds/new")} className="mt-4">
                <Plus className="mr-2 h-4 w-4" />
                İlk RSS Feed&apos;i Ekle
              </Button>
            )}
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    <div className="mb-2 flex items-center gap-2">
                      ...
                      {feed.category && <Badge variant="outline">{feed.category.name}</Badge>}
                    </div>
                    <CardDescription className="break-all">{feed.url}</CardDescription>
                    {feed.description && (
                      <p className="text-muted-foreground mt-2 text-sm">{feed.description}</p>
                    )}
                    <div className="text-muted-foreground mt-3 flex items-center gap-4 text-sm">
                      ...
                      {feed.lastScannedAt && (
                        <span>Son: {new Date(feed.lastScannedAt).toLocaleDateString("tr-TR")}</span>
                      )}
                    </div>
                    {feed.totalScans > 0 && (
